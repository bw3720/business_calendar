...
- "오늘의 할 일" 카드에서 하단 "산출 완료" 체크박스 하나 대신, 반복 업무 항목마다 우측에 개별 체크박스를 붙임 (`TodayTodo.jsx`, `App.jsx`, `App.css`). 완료 체크 시 해당 항목에 취소선 표시
- 반복 업무는 매달 재사용되는 템플릿이라 그 자체엔 완료 여부를 저장할 수 없어서, `(반복 업무, 날짜)` 조합별 완료 상태를 저장하는 `recurring_task_completions` 테이블 신설 (`supabase.sql`). `getMonthDays`가 완료 상태 맵을 받아 각 날짜의 반복 업무에 `completed`를 부여하도록 확장 (`businessDays.js`)
- 메모(자유 텍스트)는 체크박스 없이 기존처럼 노트로만 표시
